...
class FMUL_ReusedTest extends AnyFlatSpec with ChiselScalatestTester {
  import TestArgs._
...
//        _.a -> "b0000000000010000".U,
  ...
  it should "FMULReused_raw Operations" in {
    test(new FMULReused(expWidth, precision, new TestFPUCtrl(depthWarp, softThread))).withAnnotations(Seq(WriteVcdAnnotation)) { d =>
      ...
      d.io.in.bits.poke(subModuleInputInt8(1.0f, 2.0f, 0.0f, FN_FMUL))
//      d.clock.step(1)
